...
def _compile_fx_inner(
    ...
    **graph_kwargs: Unpack[_CompileFxKwargs],
    ...
    """
    aot_mode: bool = V.aot_compilation or (torch.compiler._get_fullgraph_package() is not None)
                    ...
                    if aot_mode:
                        ...
                        if p := torch.compiler._get_fullgraph_package():
                            ...
                            if isinstance(compiled_fn, list):
                                current_callable = next(fn for fn in compiled_fn if fn.endswith(".so"))
                            else:
                                current_callable = compiled_fn
    
                            if graph.device_type.startswith("cuda"):
                                current_callable = torch._C._aoti.AOTIModelContainerRunnerCuda(current_callable, 1, graph.device_type).run
                            elif graph.device_type == "cpu":
                                current_callable = torch._C._aoti.AOTIModelContainerRunnerCpu(current_callable, 1).run
                            elif graph.device_type == "xpu":
                                current_callable = torch._C._aoti.AOTIModelContainerRunnerXpu(current_callable, 1, graph.device_type).run
                            else:
                                raise RuntimeError(f"Unsupported device type {graph.device_type}")
