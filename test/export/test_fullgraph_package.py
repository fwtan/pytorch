...
class TestFullgraphPackage(TestCase):
    def setUp(self):
        if not os.path.exists(os.path.expandvars(f"/tmp/torchinductor_$USER/")):
            os.makedirs(os.path.expandvars(f"/tmp/torchinductor_$USER/"))
    ...
    def test_fullgraph_package_basic_function(self):
        ...
        torch._dynamo.reset()
        torch.compiler._load_fullgraph_package(
            path=self.path(), names=[self.id()]
        )
